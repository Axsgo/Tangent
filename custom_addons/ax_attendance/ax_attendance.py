...
class AxAttendance(models.Model):
	...
	def _compute_timesheet_hours(self):
		for rec in self:
			if rec.line_ids:
				rec.timesheet_hours = sum([x.worked_hours for x in rec.line_ids])
			else:
				rec.timesheet_hours = 0
	...
	def _compute_actual_hours(self):
		for attendance in self:
			if attendance.check_out and attendance.check_in:
				...
				attendance.actual_hours = False

	@api.depends('actual_hours','timesheet_hours')
	def _compute_worked_hours(self):
		for rec in self:
			rec.worked_hours = rec.actual_hours - rec.timesheet_hours
